...
func (ics *insertClausesSuite) TestCols() {
	c := exp.NewInsertClauses()
	cle := exp.NewColumnListExpression(nil, "db", "a", "b")
	c2 := c.SetCols(cle)
...
func (ics *insertClausesSuite) TestColsAppend() {
	cle := exp.NewColumnListExpression(nil, "db", "a")
	cle2 := exp.NewColumnListExpression(nil, "db", "b")
	c := exp.NewInsertClauses().SetCols(cle)
	c2 := c.ColsAppend(cle2)

	ics.Equal(cle, c.Cols())

	ics.Equal(exp.NewColumnListExpression(nil, "db", "a", "b"), c2.Cols())
}
