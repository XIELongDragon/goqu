...
func (tds *truncateDatasetSuite) TestGetClauses() {
	ds := goqu.Truncate("test")
	ce := exp.NewTruncateClauses().SetTable(exp.NewColumnListExpression(nil, "db", goqu.I("test")))
	...
	tds.assertCases(
		truncateTestCase{
			...
			clauses: exp.NewTruncateClauses().
				SetTable(exp.NewColumnListExpression(nil, "db", "test2")),
		},
		truncateTestCase{
			ds: bd.Table("test1", "test2"),
			clauses: exp.NewTruncateClauses().
				SetTable(exp.NewColumnListExpression(nil, "db", "test1", "test2")),
		},
		truncateTestCase{
			ds: bd,
			clauses: exp.NewTruncateClauses().
